...
在浏览器中打开：
```
http://localhost:5001/html/index.html
```
...
```bash
# 使用 Gunicorn 在后台运行
gunicorn -w 4 -b 0.0.0.0:5001 py.app:app
```
...
### 测试翻译功能
```bash
curl -X POST http://localhost:5001/api/ask/translate \
...
```bash
# 列出所有笔记
curl http://localhost:5001/api/notes/list \
  -H "Authorization: Bearer YOUR_TOKEN"

# 保存笔记
curl -X POST http://localhost:5001/api/notes/save \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_TOKEN" \
...
```bash
# 下载文件（会自动处理中文文件名）
curl http://localhost:5001/api/cloud_disk/download/1?user_id=1 \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -o downloaded_file
```
