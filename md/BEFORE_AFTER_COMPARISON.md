...
| 功能 | 修复前 | 修复后 | 改进 |
|------|------|------|------|
| **API连接** | `http://[::]:5001` 格式可能不兼容 | 自动检测 `http://localhost:5001` | ✅ 兼容更多浏览器 |
...
```
用户操作:
1. 打开 http://localhost:5001/html/index.html
2. 完成登录
3. 点击云盘按钮
...
修复后（清晰明了）:
```
云盘初始化中... API_BASE_URL: http://localhost:5001
开始验证Token... 21
验证Token响应状态: 200
...
**修复前**:
```javascript
const API_BASE_URL = 'http://[::]:5001';  // 固定IPv6格式
```
问题:
- 只支持IPv6
- 某些浏览器可能不识别IPv6格式
...
**修复后**:
```javascript
const API_BASE_URL = `http://${window.location.hostname}:5001`;  // 动态检测
```
优点:
- 自动适配IPv4和IPv6
- 与浏览器访问方式保持一致
